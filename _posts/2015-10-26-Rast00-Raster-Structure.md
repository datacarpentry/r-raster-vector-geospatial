...
> NAMES: File and object names should be make it clear what is in the file, not
> only the data but also the source of the data.  All of the data we are
> currently using come from Harvard Forest so we'll use a naming conention of
> data_HARV.
    ...
    ## Error in plot(DSM_HAV, main = "NEON Digital Surface Model\nHarvard Forest"): error in evaluating the argument 'x' in selecting a method for function 'plot': Error: object 'DSM_HAV' not found
...
other structure and attribute data when we called the object `DSM_HARV`.
...
>If you enter `DSM_HARV@data`, you'll see that units are not specified for these data.
...
always calculated and included. If they are not calculated, you can calculate
...
##Raster Bands
When we looked at the data in `DSM_HARV` we were looking only at a model of the 
surface elevation.  There was only a single piece of data--elevation in meters-- 
given for each cell.  The `DSM_HARV` raster is a single-band raster. We know that 
intuitively based on the data the raster is depicting and, also when we looked 
at the attributes and metadata (`DSM_HARV`) it said band: 1.
...
Using the same DSM file we've already used for illustration: 
