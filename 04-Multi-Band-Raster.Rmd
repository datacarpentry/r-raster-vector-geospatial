...
>Bonus: Bands? Layers?  Why are both names used? The term bands comes from the 
>fact that the data contained within each *band* comes from a different region or
>band of the electromagnetic spectrum.  When we are using multiple bands to 
>create images we layer on band on top of another, hence, the name layers.  In a 
>GIS context layers can things other than bands.  In Lesson 00 we added a lines 
>shapefile and a polygon shapefile in one image.  One could say that the line 
>layer was added to the polygon layer in the plot.  
...
>Note: In a typical GIS application, a single band would render as a single image
>band using a grayscale color palette. We will thus use a grayscale palette to 
...
>determined using the `nbands` slot. Syntax is `ObjectName@file@nbands`, or >specifically for our file: `RGB_band1@file@nbands`.
...
We can use the raster function to import specific bands in our raster object by specifying which band we want with `band=N` where N is the number of the band that you want.  If we want to import the green band we woudld use `band=2`.
...
#plot band 2
plot(RGB_band2,
     col=grayscale_colors,   #we alredy created this palate & can use it again
...
Recall from our creation of the greyscale palette that greyscale is actually a 
mix of black colors (low value) and white colors (high value). Given that most 
trees appear green we'd expect a *brighter* value for the forest in band 2 
(green) and in band 1 (red).  Therefore the larger (brighter) values appear 
closer to the white than to black giving the overall image a paler appearance.  
...
To further explore the rasters, we can use `RGB_stack@layers` to get a 
...
#view histogram of all 3 layers
hist(RGB_stack)
...
#plot all three bands seperately
plot(RGB_stack, 
     col=grayscale_colors)
...
exagerates the raster data values in order to increase contrast. 
...
>the historgram of the values ("hist") to get more contrast, but a less realistic
>image. 
...
>multiple files. If you want to know more about how to do this, check out the >`Working with multiple rasters using Raster Stacks and Raster Bricks` section 
...
Both `RasterStack` and `RasterBrick` are `R` objects with multiple layers. In 
`RasterStack` those layers can be stored anywhere on your computer. A 
...
1) What methods you can call on the `RGB_stack` object? 
2) What methods are available for a single band within `RGB_stack`? 
3) Why do you think there is a difference? 

``` {r challenge-calling-methods-code}
#1
#methods for calling a stack
methods(class=class(RGB_stack))
# 143 methods!

#2
#methods for calling a band (1) with a stack
methods(class=class(RGB_stack[1]))
