...
and the `DTM`.
This data product is often referred to as a Canopy Height Model (`CHM`) and 
represents the actual height of trees, buildings, etc, above the ground.
...
Let's create an layered map of the Digital Terrain Model (`DTM`) that includes 
hill shading (`DTM_hill`).
...
defined CRS. If unsure, we could check using `crs(DTM_hill)`.

The syntax for `projectRaster()` is 
`projectRaster(RasterObject,crs=CRSToReprojectTo)`.  Since we want to CRS to 
match the `DTM` raster we can simple tell `R` to use the CRS from DTM as the CRS
for this reprojetion.
...
First, we can see the the CRS of DTM_hillUTMZ18N did change and is now in UTMs. 
In DTM_hill where units were not defined (`no_defs`), DTM_hill_UTMZ18N now has 
meters for units.  Notice also that dimensions and extent have also changed.  
Why do you think this is?  

#### LESSON NEED TO ADD: Why do min/ max values change.  CRS shouldn't effect elevation? Is this due to changeing # of cells and then having to re-sample cells? We loose just a bit of the extremes. 
#### okay, I think this note would be better here, and expanded to address my question above. 
...
When we looked at the attributes of DTM_hill_UTMZ18N we should have noticed that
the resolution for the reprojected raster was 1 x 0.998. Yet, we know that 
...
And now it works.  Now that we can layer both the Digital Terrain Model and the 
Hillshade of the DTM we can now continue on to Lesson 03 where we'll learn to 
manipulate layers in order to create the Canopy Height Model that we are after. 