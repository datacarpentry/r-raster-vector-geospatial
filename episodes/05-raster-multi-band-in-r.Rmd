...
collected using the
[NEON Airborne Observation Platform](http://www.neonscience.org/science-design/collection-methods/airborne-remote-sensing)
high resolution camera over the
[NEON Harvard Forest field site](http://www.neonscience.org/science-design/field-sites/harvard-forest).
...
```{r}
RGB_band1_HARV_df  <- raster::as.data.frame(RGB_band1_HARV, xy = TRUE)
```
...
We can conver this data to a data frame and plot the same way we plotted the red band: 

```{r}
RGB_band2_HARV_df  <- raster::as.data.frame(RGB_band2_HARV, xy = TRUE)
```
...
```{r}
RGB_stack_HARV_df  <- raster::as.data.frame(RGB_stack_HARV, xy = TRUE)
```
...
> ## More Resources
>[More on Raster Bricks](http://www.inside-r.org/packages/cran/raster/docs/brick)
{: .callout}
...
> > ```
> > 2) And compare that will the methoda available for a single band:
> > ```{r}
