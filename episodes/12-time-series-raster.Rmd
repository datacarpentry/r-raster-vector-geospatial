...
- "Pplot and explore time series raster data."
keypoints:
- ""
---
...
We introduced the concepts of NDVI and RGB data in [an earlier lesson](https://datacarpentry.org/organization-geospatial/01-spatial-data-structures-formats/index.html).

![A multi-band raster dataset can contain time series data. Source: National Ecological Observatory Network (NEON)](../images/dc-spatial-raster/GreennessOverTime.jpg)
...
### Getting Started
In this episode, we will use the `raster`, `rgdal`, `reshape`, and `scales` libraries. Make sure you have these libraries loaded.
...
the `ggplot()` command to create a multi-panelled plot showing each band in our `RasterStack`.
...
```{r}
ggplot(yr_11_daily_avg,aes(jd, airt)) +
    geom_point() +
    ggtitle("Daily Mean Air Temperature", subtitle = "NEON Harvard Forest Field Site") +
    xlab("Julian Day 2011") + ylab("Mean Air Temperature (C)")
```
...
> > 
> > ```{r}
> > RGB_277_df <- raster::as.data.frame(RGB_277, xy = TRUE)
> > ```
> > 
...
> > RGB_293 <- RGB_293/255
> > RGB_293_df <- raster::as.data.frame(RGB_293, xy = TRUE)
