...
RGB_133_df <- RGB_133_df %>%
    mutate(r = quantile(X133_HARV_landRGB.1, quantiles, na.rm = TRUE),
           tempR = (X133_HARV_landRGB.1 - r[1])/(r[2] - r[1]),
           tempR = case_when(
             tempR < 0 ~ 0,
             tempR > 1 ~ 1,
             TRUE ~ tempR),
           g = quantile(X133_HARV_landRGB.2, quantiles, na.rm = TRUE),
           tempG = (X133_HARV_landRGB.2 - g[1])/(g[2] - g[1]),
           tempG = case_when(
             tempG < 0 ~ 0,
             tempG > 1 ~ 1,
             TRUE ~ tempG),
           b = quantile(X133_HARV_landRGB.3, quantiles, na.rm = TRUE),
    ...
    dplyr::select(-(r:tempB))  # get rid of the temporary variables we created.
ggplot() +
  geom_raster(data = RGB_133_df, aes(x, y), fill = rgb) + 
...
RGB_197_df <- as.data.frame(RGB_197, xy = TRUE) %>%
    mutate(r = quantile(X133_HARV_landRGB.1, quantiles, na.rm = TRUE),
           tempR = (X197_HARV_landRGB.1 - r[1])/(r[2] - r[1]),
           tempR = case_when(
             tempR < 0 ~ 0,
             tempR > 1 ~ 1,
             TRUE ~ tempR),
           g = quantile(X197_HARV_landRGB.2, quantiles, na.rm = TRUE),
           tempG = (X197_HARV_landRGB.2 - g[1])/(g[2] - g[1]),
           tempG = case_when(
             tempG < 0 ~ 0,
             tempG > 1 ~ 1,
             TRUE ~ tempG),
           b = quantile(X197_HARV_landRGB.3, quantiles, na.rm = TRUE),
    ...
    dplyr::select(-(r:tempB)) # get rid of the temporary variables we created.
ggplot() +
  geom_raster(data = RGB_197_df, aes(x, y), fill = rgb) + 
