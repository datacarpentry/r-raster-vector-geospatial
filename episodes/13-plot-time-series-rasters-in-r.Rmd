...
> > function. The 
> > parameter to set is called `face`. 
> > 
> > ```{r}
> > ggplot() +
> >   geom_raster(data = NDVI_HARV_stack_df , aes(x = x, y = y, fill = value)) +
...
We can 
tell `colorRampPalette` how many discrete colors within this color range to
...
assign this output to a new object (`rasterNames`) and look
at that object to make sure our code is doing what we want it to.

```{r}
rasterNames <- names(NDVI_HARV_stack)
...
```{r}
rasterNames  <- gsub("X", "Day ", rasterNames)
rasterNames
```
...
Lastly, let's make sure that our x and y coordinates are equal. 
