...
metadata/raster attributes that are needed to work with raster data in `R`. We
discuss the three core metadata elements that we need to understand to work with
rasters in `R`: CRS, extent and resolution. We will also explore missing and bad
data values as stored in a raster and how `R` handles these elements. Finally,
we will introduce the GeoTiff file format.

To open rasters in `R`, we will use the `raster` and `rgdal` packages. Make sure
that you have these libraries loaded.
...
We will be exploring this data throughout this episode. By the end of this
...
raster dataset into `R` and explore its metadata more closely.
...
you can try tuning the parameter `maxsamp`:

```{r }
# lets try all cells instead
...
`raster` has an built-in function for conversion to a plotable `data.frame`.
...
> ## Data Tip
> 
> There are many user contributed packages for `R`, each with its own set of functions. 
> Functions in different packages can have the same name, even though they do different things. 
> The notation `package_name::function_name` tells `R` which package library you want 
> it to look in for a particular function. In this case, this notation is required, because
> the base `R` set of functions (which is always searched first unless otherwise specified) 
> also includes an `as.data.frame()` function.
{: .callout}
...
> ## Data Tip
> The add-on package `ggspatial` can also be used. The raster is still converted
> to a dataframe before plotting, but the work of converting spatial data into a
> ggplot-friendly format is done for you in the background.
> 
> ```{r 'ggspat', eval = FALSE}
> library(ggspatial)
> ggplot() +
...
lesson](https://datacarpentry.org/organization-geospatial/01-spatial-data-structures-formats/index.html).
...
![The UTM zones across the continental United States. Source: Chrismurf wikimedia.org.](https://en.wikipedia.org/wiki/File:Utm-zones-USA.svg)
...
![Source: National Ecological Observatory Network (NEON)](../images/dc-spatial-raster/single_multi_raster.png)
...
# convert to a df for plotting using raster's built-in methods
RGB_2m_df  <- raster::as.data.frame(RGB_2m, xy = TRUE)
...
To highlight NA values in gpplot, alter the `scale_fill_*()` layer to contain a colour instruction for na values, like `scale_fill_viridis_c(na.value = 'deeppink')`
...
the figure `-3.4e+38` is a common default, and for integers, `-9999` is also
frequently seen. Some disciplines have specific conventions that vary from these
common values.
