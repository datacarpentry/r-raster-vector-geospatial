...
We often work with spatial layers that have different spatial extents. The spatial extent of a shapefile or R
spatial object represents the geographic "edge" or
location that is the furthest north, south east and
west. Thus is represents the overall geographic
...
that falls within the boundaries of the AOI. Remember,
since this is raster data, we need to convert to 
...
ggplot() +
  ...
  geom_raster(data = CHM_HARV_Crop_df, aes(x = x, y = y, fill = HARV_chmCrop)) + 
     scale_fill_gradientn(name = "Canopy Height", colors = terrain.colors(10)) + 
     coord_sf()
```
...
Our plot location extent is not the
...
ggplot() + 
    geom_sf(data = aoi_boundary_HARV, color = "blue", fill = NA) +
    geom_raster(data = CHM_HARV_manualCrop_df, aes(x = x, y = y, fill = HARV_chmCrop)) + 
    scale_fill_gradientn(name = "Canopy Height", colors = terrain.colors(10)) + 
    coord_sf()
```
...
![Extract raster information using a polygon boundary.](https://www.neonscience.org/sites/default/files/images/spatialData/BufferSquare.png)
...
![Extract raster information using a buffer region.](https://www.neonscience.org/sites/default/files/images/spatialData/BufferCircular.png)
...
> > 
> > # plot data
> > ggplot(data = meanTreeHt_plots_HARV, aes(ID, HARV_chmCrop)) + 
> >   geom_col() + 
