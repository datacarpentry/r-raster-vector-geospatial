...
After subtracting, change the column name from "layer" to be more 
descriptive, "HARV_chmCrop", and create a dataframe so we can plot 
with `ggplot`.

```{r raster-math }
CHM_HARV <- DSM_HARV - DTM_HARV
names(CHM_HARV)
names(CHM_HARV)[1] <- "HARV_chmCrop"
...
```{r create-hist}
ggplot(CHM_HARV_df) +
    geom_histogram(aes(HARV_chmCrop))
```
...
> > `na.rm = TRUE`. 
> > ```{r}
> > min(CHM_HARV_df$HARV_chmCrop, na.rm = TRUE)
> > max(CHM_HARV_df$HARV_chmCrop, na.rm = TRUE)
> > ```
...
> > ggplot(CHM_HARV_df) +
> >     geom_histogram(aes(HARV_chmCrop), colour="black", 
...
> >                   mutate(canopy_discrete = cut(HARV_chmCrop, breaks = custom_bins))
> > 
> > ggplot() +
...
names(CHM_ov_HARV)[1] <- "HARV_chmCrop"                       
```
...
> >                        DTM_SJER,
> >                        fun = function(r1, r2){ return(r1 - r2) })
> > names(CHM_ov_SJER)[1] <- "SJER_chmCrop"                       
> > ```
...
> >     geom_histogram(aes(SJER_chmCrop))
> > ```
> >
