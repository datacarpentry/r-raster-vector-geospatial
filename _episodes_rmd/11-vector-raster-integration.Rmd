...
ggplot() +
  ...
  geom_raster(data = CHM_HARV_Crop_df, aes(x = x, y = y, fill = layer)) + 
     scale_fill_gradientn(name = "Canopy Height", colors = terrain.colors(10)) + 
  ggtitle("LiDAR CHM - Cropped", subtitle = "NEON Harvard Forest Field Site")
```
...
> >   ggtitle("Study Plot Locations", subtitle = "NEON Harvard Forest")
> > ```
> {: .solution}
{: .challenge}
...
ggplot() + 
    ...
    scale_fill_gradientn(name = "Canopy Height", colors = terrain.colors(10)) + 
    ggtitle("NEON Harvard Forest", subtitle = "Manually cropped")
```
...
boundaries,
* we can tell it to store the output values in a dataframe using
...
ggplot() + 
  ...
  ggtitle("Histogram of CHM Height Values (m)", subtitle = "NEON Harvard Forest Field Site") +
  xlab("Tree Height") + 
...
![Extract raster information using a buffer region. All pixels that are touched by the buffer region are included in the extract. Source: National Ecological Observatory Network (NEON).](https://www.neonscience.org/sites/default/files/images/spatialData/BufferCircular.png)
