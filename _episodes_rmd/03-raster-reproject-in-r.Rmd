...
Now we can create a map of the DTM layered over the hillshade. (Suggest that the instructor goes over this steps, but tell the class to just follow along.)
...
Let's try to
plot the DTM on its own to make sure there are data there. (Suggest that the instructor goes over this steps, but tell the class to just follow along.)
...
Next we plot the DTM Hillshade on its own to see whether everything is OK. (Suggest that the instructor goes over this steps, but tell the class to just follow along.)
...
the hillshade data to see how they differ with the `crs()` function.
...
res(DTM_hill_HARV) # see suggested edit in different commit
```
...
DTM_hill_UTMZ18N_HARV <- projectRaster(DTM_hill_HARV,
                                  crs = crs(DTM_HARV),
                                  res = 1) # see suggested edit in different commit
```
...
(Suggest to skip challenge in interest of time)
