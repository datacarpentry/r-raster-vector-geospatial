...
We can then convert our output to a dataframe using `as.data.frame()`. It's a good
idea to view the first few rows of our dataframe with `head()` to make sure the
...
We now have a dataframe with row names that are based on the original file name and
a mean NDVI value for each file. Next, let's clean up the column names in our
dataframe to make it easier for colleagues to work with our code.
...
Day value, just like we did in the [last episode]({{ site.baseurl }}/13-plot-time-series-rasters-in-r/).
...
> at the
> [San Joaquin Experimental Range (SJER)](http://www.neonscience.org/science-design/field-sites/san-joaquin-experimental-range).
> 
